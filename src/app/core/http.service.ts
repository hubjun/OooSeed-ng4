...
export class HttpService {
   ...
   *  关于user类型的接口都需要配置该信息
   */
  public setHeader(urlEncode: boolean = false): RequestOptionsArgs {
    ...
    if (urlEncode && urlEncode == true) {
      options.headers.set('Content-Type', 'application/x-www-form-urlencoded');
    } else {
      options.headers.set('Content-Type', 'text/json');
    }
    options.headers.set("X-SNS-ClientType", '4');
  ...
  public postUrlencode(url: string, data: any): Observable<Response> {
    let URL = this.APP_HOST + url;
    let body = this.tools.param(data);
    let option = this.setHeader(true)
    return this.http.post(URL, body, option)
  }
  ...
  public upload(url: string, file: any): Observable<Response> {
    ...
    let headers = this.setHeader(true);
